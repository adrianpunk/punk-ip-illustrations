...
```text
欢迎使用 Punk IP 插图。整个流程分为四步：
1. 上传一张人物清晰的照片，并告诉我角色名称或昵称；
2. 我会生成角色设定板和干净人物参考图；
3. 你确认或提出修改，确认后角色会被保存；
4. 再上传文章，我会自动判断配图数量和表现模式。

请先上传一张照片，并告诉我角色名称或昵称。我会先创建你的 IP 形象，等你确认后再进入文章配图。
```
...
已经存在确认角色时，Skill 不会重复要求上传照片，而是直接提示使用当前角色并请你提供文章。
